...
async function main() {
  const { data: apps } = await supabase.admin.from('restaurant_applications').select('id, restaurant_name, user_id');
  ...
  if (!apps || !users || !items) { console.log('ERROR: missing data'); return; }

  // Restaurant: Asan Chwok Sweets & Snacks
  ...
  console.log('-- ============================================');
  console.log('-- STEP 1: Add missing columns to orders table');
  console.log('-- ============================================\n');
  ...
  console.log('-- ============================================');
  console.log('-- STEP 2: Populate restaurants table');
  console.log('-- ============================================\n');

  for (const a of apps) {
    console.log(`INSERT INTO public.restaurants (id, name, owner_id) VALUES ('${a.id}', '${esc(a.restaurant_name)}', '${a.user_id}') ON CONFLICT (id) DO NOTHING;`);
  }

  console.log('\n-- ============================================');
  console.log('-- STEP 3: Create 6 test orders');
  console.log('-- ============================================\n');
  ...
  console.log('\n-- ============================================');
  console.log('-- STEP 4: Set timestamps for each transition');
  console.log('-- ============================================\n');
  ...
  for (const t of testOrders) {
    const updates = Object.entries(t.ts)
      ...
      .join(', ');
    if (updates) console.log(`UPDATE public.orders SET ${updates}, updated_at = NOW() WHERE order_number = '${t.num}';`);
  }
  ...
  console.log('-- ============================================\n');
  console.log("SELECT order_number, status, total, created_at FROM public.orders WHERE order_number LIKE 'LIFECYCLE-%' ORDER BY order_number;");
