...
/// Admin-facing screen for managing ALL coupons.
/// Shows every coupon across all restaurants + global coupons.
/// Admins can create global coupons and delete any coupon.
///
/// When [embedded] is true (used inside a TabBarView), the Scaffold/AppBar
/// is omitted to avoid nesting with the parent's AppBar.
class AdminCouponManagementScreen extends StatefulWidget {
  final bool embedded;

  const AdminCouponManagementScreen({super.key, this.embedded = false});
...
class _AdminCouponManagementScreenState
  ...
  Future<void> _fetchCoupons() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _coupons = coupons;
          _totalCount = coupons.length;
          _activeCount = coupons.where((c) => c['is_active'] == true).length;
          _globalCount = coupons.where((c) => c['restaurant_id'] == null).length;
          _isLoading = false;
        });
      }
  ...
  Future<void> _deleteCoupon(String id) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete this coupon? This cannot be undone.'),
        actions: [
  ...
  Future<void> _showCreateCouponSheet() async {
    final result = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (ctx) => const _AdminCreateCouponSheet(),
    );
    ...
    if (result != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Global coupon created successfully!'),
    ...
    if (widget.embedded) {
      return Scaffold(
        ...
        floatingActionButton: FloatingActionButton.extended(
          ...
          label: const Text('Global Coupon'),
        ),
        body: _buildBody(),
      );
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _buildStatCard(Icons.local_offer_rounded, 'Total', '$_totalCount', const Color(0xFF1967D2))),
              const SizedBox(width: 10),
              Expanded(child: _buildStatCard(Icons.check_circle_rounded, 'Active', '$_activeCount', const Color(0xFF1E8E3E))),
              const SizedBox(width: 10),
              Expanded(child: _buildStatCard(Icons.public_rounded, 'Global', '$_globalCount', const Color(0xFFF9A825))),
            ],
          ),
          const SizedBox(height: 16),
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    const Text('Create a global coupon to offer site-wide discounts',
            ...
            children: [
              ...
              if (isGlobal) ...[
                ...
                Container(
                  ...
                  child: const Text(
                    'Restaurant',
                    style: TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: Color(0xFF5C5C5C)),
                  ),
                ),
              ],
              if (!isActive) ...[
...
// ──────────────────────────────────────────────
//  Admin Create Global Coupon Bottom Sheet
// ──────────────────────────────────────────────

class _AdminCreateCouponSheet extends StatefulWidget {
  const _AdminCreateCouponSheet();

  @override
  State<_AdminCreateCouponSheet> createState() => _AdminCreateCouponSheetState();
}

class _AdminCreateCouponSheetState extends State<_AdminCreateCouponSheet> {
      ...
      child: Column(
        children: [
          ...
          const Padding(
            ...
            child: Text('Create Global Coupon', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
          ),
          // Info badge
          Container(
            ...
            child: const Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(Icons.public, size: 14, color: Color(0xFF1967D2)),
                SizedBox(width: 6),
                Text(
                  'This coupon will be available across all restaurants',
                  style: TextStyle(fontSize: 11, color: Color(0xFF1967D2), fontWeight: FontWeight.w500),
                ),
              ],
            ),
          ),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  _buildTextField(controller: _codeCtrl, hint: 'e.g. SAVE20', textCapitalization: TextCapitalization.characters),
                  const SizedBox(height: 16),
                  ...
                  const SizedBox(height: 6),
                  _buildTextField(controller: _minOrderCtrl, hint: 'e.g. 500', keyboardType: TextInputType.number, prefix: 'Rs. '),
                  const SizedBox(height: 16),

                  if (_discountType == 'PERCENTAGE') ...[
                    _buildLabel('Maximum Discount Cap (optional)'),
                    const SizedBox(height: 6),
                    _buildTextField(controller: _maxCapCtrl, hint: 'e.g. 200', keyboardType: TextInputType.number, prefix: 'Rs. '),
                    const SizedBox(height: 16),
                  ],

                  _buildLabel('Usage Limit (optional)'),
                  const SizedBox(height: 6),
                  _buildTextField(controller: _usageLimitCtrl, hint: 'e.g. 100', keyboardType: TextInputType.number),
                  const SizedBox(height: 16),
                  ...
                  const SizedBox(height: 6),
                  _buildTextField(controller: _descriptionCtrl, hint: 'e.g. Flat 20% off site-wide', maxLines: 2),
                  const SizedBox(height: 24),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: _isCreating
                          ...
                          : const Text('Create Global Coupon', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                    ),
                  ),
                ],
              ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: isSelected ? const Color(0xFFBB0018) : const Color(0xFFE5E7EB)),
        ),
        child: Text(
          label,
          style: TextStyle(
